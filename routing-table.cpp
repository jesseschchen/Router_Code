...
RoutingTable::lookup(uint32_t ip) const
{
	...
	{
		//std::cerr<<"checking: "<<ipToString(entry.dest)<<"\n";
		//std::cerr<<ipToString(entry.mask & entry.dest) << "=?="<<ipToString(entry.mask & hip)<<"\n";
		if((entry.mask & entry.dest) == (entry.mask & ip))
		{
			if(out.mask <= entry.mask)
			{
				//std::cerr<<ipToString(out.mask) <<" < "<<ipToString(entry.mask)<<"\n";
				out = entry;
				match = true;
				//std::cerr<<"new king\n\n";				
			}
